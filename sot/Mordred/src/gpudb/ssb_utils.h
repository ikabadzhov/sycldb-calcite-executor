...
#define BASE_PATH "/media/ivan/SYCLDB/FULLENG/sycldb-calcite-executor/sot/Mordred/data/"
...
T* loadColumn(string col_name, int num_entries) {
  T* h_col = new T[((size_t)((num_entries + SEGMENT_SIZE - 1)/SEGMENT_SIZE)) * SEGMENT_SIZE];
  ...
  colData.read((char*)h_col, (size_t)num_entries * sizeof(T));
  return h_col;
}
...
int storeColumn(string col_name, int num_entries, int* h_col) {
  ...
  colData.write((char*)h_col, (size_t)num_entries * sizeof(T));
  return 0;
}
