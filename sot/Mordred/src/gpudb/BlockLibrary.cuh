...
__device__ __forceinline__ void BlockProbeGPU2Direct(
  ...
  for (int ITEM = 0; ITEM < ITEMS_PER_THREAD; ++ITEM)
  {
    ...
    if (selection_flags[ITEM]) {  
      ...
      int key = gpuCache[(size_t)dimkey_seg * SEGMENT_SIZE + (items_off[ITEM] % SEGMENT_SIZE)];
...
__device__ __forceinline__ void BlockReadFilteredOffsetDirect(
  ...
  {
    if (selection_flags[ITEM]) {
      items[ITEM] = gpuCache[(size_t)col_idx[items_off[ITEM] / SEGMENT_SIZE] * SEGMENT_SIZE + (items_off[ITEM] % SEGMENT_SIZE)];
    }
  }
}
...
__device__ __forceinline__ void BlockBuildValueGPU2Direct(
  ...
  for (int ITEM = 0; ITEM < ITEMS_PER_THREAD; ++ITEM)
  {
    if (selection_flags[ITEM]) {
      ...
      int key = gpuCache[(size_t)dimkey_seg * SEGMENT_SIZE + (items_off[ITEM] % SEGMENT_SIZE)];
      int val = gpuCache[(size_t)dimval_seg * SEGMENT_SIZE + (items_off[ITEM] % SEGMENT_SIZE)];
