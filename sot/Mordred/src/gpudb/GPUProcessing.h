...
__global__ void probe_group_by_GPU2(
  ...
  if (pargs.key_idx1 != NULL && pargs.ht1 != NULL) { //normal operation, here pargs.key_idx will be lo_partkey, lo_suppkey, etc (the join key column) -> no group by attributes
    ptr = gpuCache + (size_t)key_segment1 * SEGMENT_SIZE;
  ...
  if (pargs.key_idx2 != NULL && pargs.ht2 != NULL) {
    ptr = gpuCache + (size_t)key_segment2 * SEGMENT_SIZE;
    BlockLoadCrystal<int, BLOCK_THREADS, ITEMS_PER_THREAD>(ptr + segment_tile_offset, items, num_tile_items);
  ...
  if (pargs.key_idx3 != NULL && pargs.ht3 != NULL) {
    ptr = gpuCache + (size_t)key_segment3 * SEGMENT_SIZE;
    BlockLoadCrystal<int, BLOCK_THREADS, ITEMS_PER_THREAD>(ptr + segment_tile_offset, items, num_tile_items);
  ...
  if (pargs.key_idx4 != NULL && pargs.ht4 != NULL) {
    ptr = gpuCache + (size_t)key_segment4 * SEGMENT_SIZE;
    BlockLoadCrystal<int, BLOCK_THREADS, ITEMS_PER_THREAD>(ptr + segment_tile_offset, items, num_tile_items);
  ...
  if (gargs.aggr_idx1 != NULL) {
    ptr = gpuCache + (size_t)aggr_segment1 * SEGMENT_SIZE;
  ...
  if (gargs.aggr_idx2 != NULL) {
    ptr = gpuCache + (size_t)aggr_segment2 * SEGMENT_SIZE;
...
__global__ void filter_probe_GPU2(
  ...
  if (fargs.filter_idx1 != NULL) {
    ptr = gpuCache + (size_t)filter_segment1 * SEGMENT_SIZE;
    BlockLoadCrystal<int, BLOCK_THREADS, ITEMS_PER_THREAD>(ptr + segment_tile_offset, items, num_tile_items);
  ...
  if (fargs.filter_idx2 != NULL) {
    ptr = gpuCache + (size_t)filter_segment2 * SEGMENT_SIZE;
    BlockLoadCrystal<int, BLOCK_THREADS, ITEMS_PER_THREAD>(ptr + segment_tile_offset, items, num_tile_items);
...
__global__ void build_GPU2(
  ...
  if (fargs.filter_idx1 != NULL) {
    int* ptr = gpuCache + (size_t)filter_segment * SEGMENT_SIZE;
    BlockLoadCrystal<int, BLOCK_THREADS, ITEMS_PER_THREAD>(ptr + segment_tile_offset, items, num_tile_items);
  ...
  int* ptr_key = gpuCache + (size_t)key_segment * SEGMENT_SIZE;
  ...
  if (bargs.val_idx != NULL) {
    int* ptr = gpuCache + (size_t)val_segment * SEGMENT_SIZE;
...
__global__ void filter_GPU2(
  ...
  if (fargs.filter_idx1 != NULL) {
    int* ptr = gpuCache + (size_t)filter_segment1 * SEGMENT_SIZE;
    BlockLoadCrystal<int, BLOCK_THREADS, ITEMS_PER_THREAD>(ptr + segment_tile_offset, items, num_tile_items);
  ...
  if (fargs.filter_idx2 != NULL) {
    int* ptr = gpuCache + (size_t)filter_segment2 * SEGMENT_SIZE;
    BlockLoadCrystal<int, BLOCK_THREADS, ITEMS_PER_THREAD>(ptr + segment_tile_offset, items, num_tile_items);
