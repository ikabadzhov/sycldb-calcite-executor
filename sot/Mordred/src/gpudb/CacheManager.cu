...
CacheManager::CacheManager(size_t _cache_size, size_t _processing_size, size_t _pinned_memsize) {
	...
	pinnedMemory = new uint64_t[_pinned_memsize];
	gpuPointer = 0;
	cpuPointer = 0;
	pinnedPointer = 0;
...
CacheManager::cacheSegmentInGPU(Segment* seg) {
	...
	CubDebugExit(cudaMemcpy(&gpuCache[(size_t)idx * SEGMENT_SIZE], seg->seg_ptr, (size_t)SEGMENT_SIZE * sizeof(int), cudaMemcpyHostToDevice));
