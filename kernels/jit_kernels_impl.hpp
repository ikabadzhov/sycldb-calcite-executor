...
#define JIT_IMPL_ALL_SLOTS_INLINE(name) \
    ...
    inline void selection_literal_jit_impl(sycl::item<1> idx, SYCLDBContext ctx, int i, bool& pass, uint64_t& acc) {
        if (__builtin_expect(pass, 1)) pass = logical((logical_op)ctx.params[i*8+1], pass, compare((comp_op)ctx.params[i*8+0], ctx.col_ptrs[i][idx], ctx.values[i]));
    }
    ...
    inline void selection_columns_jit_impl(sycl::item<1> idx, SYCLDBContext ctx, int i, bool& pass, uint64_t& acc) {
        if (__builtin_expect(pass, 1)) pass = logical((logical_op)ctx.params[i*8+1], pass, compare((comp_op)ctx.params[i*8+0], ctx.col_ptrs[i][idx], ctx.col_ptrs2[i][idx]));
    }
    ...
    inline void perform_op_columns_jit_impl(sycl::item<1> idx, SYCLDBContext ctx, int i, bool& pass, uint64_t& acc) {
        if (__builtin_expect(pass, 1)) {
            ctx.res_ptrs[i][idx] = element_operation(ctx.col_ptrs[i][idx], ctx.col_ptrs2[i][idx], (BinaryOp)ctx.params[i*8+2]);
        }
    }
    ...
    inline void full_join_jit_impl(sycl::item<1> idx, SYCLDBContext ctx, int i, bool& pass, uint64_t& acc) {
        if (__builtin_expect(pass, 1)) {
            ...
            bool in_range = (val >= ht_min && val <= ht_max);
            int hash = HASH(val, ht_len, ht_min) << 1;
            
            // Predicated load to avoid out of bounds if not in range
            int key = in_range ? ctx.ht_int_ptrs[i][hash] : 0;
            if (__builtin_expect(key == 1, 1)) {
    ...
    inline void aggregate_jit_impl(sycl::item<1> idx, SYCLDBContext ctx, int i, bool& pass, uint64_t& acc) {
        if (__builtin_expect(pass, 1)) acc += (uint64_t)ctx.col_ptrs[i][idx];
    }
